...
// here, noisy mode will mean that the pulse programmer loops through scans without reprogramming

// also want to have a filter mode -> fft vs boxcar ? With SOAPY, I think we'll always collect data
// at fast fixed rate, filter after the fact.

// do this (NO_RT_SCHED) till all is well tested...
*/
//#define NO_RT_SCHED
...
#include "shm_data.h"
#include "shm_prog-duesdr.h"          //indirectly dependant on h_config.h
#include "p_signals.h"
...
// comment out for testing without hardware
#define HDUEPP
#define SOAPYRX
//#define DUEBGU  // also in pulse-duesdr.c
    ...
    
{
  ...
  case ACQ_START:
	...
	*/
	...
	waiting=1;
	//	wait_till_streams_done(); // don't think this is needed...
  ...
  case ACQ_STOP:
    ...
    //    prog_shm->stop_rx = NICE_DONE; // nope - only when all is said and done.
    break;
  ...
  case ACQ_KILL:
    ...
    prog_shm->stop_rx = KILL_DONE; // tell the sdr to stop too.
...
void start_up_hardware(int *first_time,int64_t *buffers, char use_bgu){
  ...
  /// At start of acquisition, we  send the start-up sync program, run it, then send the real program.
  ...
  if (*first_time){ 
    ...
    prog_shm->etxprogno = prog_shm->txprogno; // this is the program that will be executed. only bgu cares
    prog_shm->txprogno = ( prog_shm->txprogno + 1 ) %2; // calc next one in other buffer.
#ifdef SOAPYRX
    if (done >= 0){
      rval = prep_sdr(data_shm->npts,buffers, 1) ; // get the sdrs ready to go.
      if ( rval < 0) // master lime produces the sync pulses that reset the timestamps, and starts the due pp
	done = ERROR_DONE;
#ifdef HDUEPP // need both sdr and due for this to work:??
      printf("about to call start_sdr_threads\n");
      if ( done >= 0) 
	start_sdr_threads();  // tells the sdr to prepare to start looking for sync, and then acquire data.
      usleep(100);
#endif
    }
    ...
    // we just count posts to rxwrite? should be one for each receiver?
    if(check_sync_was_found() != NUM_RX){
      done=ERROR_DONE;
      prog_shm->stop_rx = ERROR_DONE;
      printf("sync pulses not all observed, hardware problem?\n");
      send_sig_ui(RX_ERROR); 
    }
    
#ifdef SOAPYRX
    if (done >= 0){
      rval = prep_sdr(data_shm->npts,buffers, 0) ; // get the sdrs ready to go.
      if ( rval < 0) {
	done = ERROR_DONE;
	prog_shm->stop_rx = ERROR_DONE;
	printf("prep_sdr failed\n");
	send_sig_ui( RX_ERROR );
      }
    }
#endif
...
#ifdef SOAPYRX
    if (done >=0){
      rval = prep_sdr(data_shm->npts,buffers,0) ;
      if ( rval != 0){ // <0 is an error
	done = ERROR_DONE;
	prog_shm->stop_rx = ERROR_DONE;
	printf("prep_sdr failed\n");
	send_sig_ui( RX_ERROR );
...

{
  ...
  if (buffers == NULL)
    ...
    printf("WARNING: acq-duesdr, staring run, but buffers weren't NULL?!\n");
 ...
 ruid=getuid();
  ...
  if (prog_shm->is_noisy == 1){
    if (block_size == 0){ // 2e9 seems very big! Its so we don't request too many points that overflow an int.
  ...
  if (done == NOT_DONE){
#ifdef HDUEPP
    i = init_pulse_hardware(NUM_BOARDS);// finds the number of due boards requested and opens them.
  ...
  pulse_hardware_build_startup_soapy_new(start_up_prog,NUM_BOARDS); // build due startup-sync program.
  //  pulse_hardware_build_startup_soapy(start_up_prog,NUM_BOARDS); // build due startup-sync program.
  printf("returned from build_startup\n");
  ...
  //  data_shm->time_remaining = pp_time_new(&done)/(double)DUE_PP_CLOCK; // this should be the duration of the start-up sync program 
  data_shm->time_remaining = 0; // this should be the duration of the start-up sync program - nah, it never gets subtracted.
  ...
  for (i=0;i< data_shm->num_acqs_2d && done == NOT_DONE;i++){
    ...
    if (done == NOT_DONE){
      //      ppt = pp_time_new(&done)/(double)DUE_PP_CLOCK;
      ppt = prog_shm->prog_dur_in_due_ticks/(double) DUE_PP_CLOCK;
      if (done == ERROR_DONE)
	prog_shm->stop_rx = ERROR_DONE;
	...
	//	else fprintf(stderr,"in acq, calc'ing pp length, got j = %i\n",j);
      ...
      fprintf(stderr,"after calc'ing: %d, total time is: %lf\n", data_shm->acqn_2d,data_shm->time_remaining);
  ...
  printf("DONE PRECALC. time_remaining is:%f\n",data_shm->time_remaining);
  /*
   *  Start the main loop. First real program was set to calculate above.
   */
  ...
  if (prog_shm->is_noisy == 0){
	...
	if (done >=0){
	  ...
	  //	  current_total_time = pp_time_new(&done)/(double)DUE_PP_CLOCK;
	  current_total_time = prog_shm->prog_dur_in_due_ticks/(double) DUE_PP_CLOCK;
	  if (done == ERROR_DONE)
	    prog_shm->stop_rx = ERROR_DONE;
	...
	// generate RX events for thread:
	printf("acq: calling generate_rx_sdr_events\n");
        generate_rx_sdr_events(first_time);
	printf("acq: calling start up hardware\n");
	...
	//	prog_shm->rx_stamp_count += prog_shm->prog_dur_in_rx_stamps; do in generate_rx_events
	data_shm->acqn++;
	...
	if (done >=0 && ((data_shm->acqn_2d < data_shm->num_acqs_2d && data_shm->acqn < data_shm->num_acqs) || (data_shm->mode == REPEAT_MODE) )){
	  ...
	  if ((num_calced == (data_shm->num_acqs * data_shm->num_acqs_2d -1 + init_dummy_scans)) && (data_shm->mode == NORMAL_MODE || data_shm->mode == NORMAL_MODE_NOSAVE)){
	    //	    printf("acq: setting last_real_prog\n");
	...
	if( done >= 0 ) { // update time remaining, and read in data.
	  ...
	  printf("TIME REMAINING: %f, total: %f, cur ppo %f, prev ppo %f\n",data_shm->time_remaining,current_total_time,current_ppo_time,prev_ppo_time);
	  prev_ppo_time = current_ppo_time;
	  
#ifdef SOAPYRX
	  printf("starting to wait for sdr data\n");
	  i = wait_for_sdr_data(); // ok, sdr data is ready.
	  printf("done waiting for sdr data\n");
	  if (i<0) {
	    done = ERROR_DONE;
	    printf("wait_for_sdr_data came back with error, setting done to ERROR_DONE\n");
	  }
#endif
	...
	*/
	...
	/* what are all these things?
	   prog_shm->totalpoints - sum of data points for all receivers. This is a different beast from the rest.

	   total_pts number of points in the block we acquire? 
	   pts_request - we break it up for display updates - this is how many points before update
	   pts_in_request - how many points we've collected out of pts_request
	   pts_in_total - how many points we've collected on the way to total_pts
	...
	*/
	...
	// manipulate how many points the sdrs think they need to return:
	for (i=0;i<num_sdrs_found;i++){
	  if (increment+dummy_scans > 0){
	  printf("acq-duesdr noisy mode: rxpoints %i was %li. Making it: %li\n",i,prog_shm->rxpoints[i],prog_shm->rxpoints[i]/(increment+dummy_scans));
	  prog_shm->rxpoints[i] = prog_shm->rxpoints[i]/(increment+dummy_scans);
	  }
	}
	printf("acq: calling generate_rx_sdr_events\n");
        generate_rx_sdr_events(first_time); // noisy mode pretty much demands that RX is on continuously, so this should just be one event.
...
int main(){
  ...
  // open sdr devices at start:
#ifdef SOAPYRX
  printf("calling open_sdr\n");
  num_sdrs_found = open_sdr();
  if ( num_sdrs_found < 1)
    printf("No SDRs found!\n");
#endif
...
#ifndef SOAPYRX
  fprintf(stderr,"\n\n********************************\n\n");
  fprintf(stderr,"            Acq started without SOAPYRX\n");
  fprintf(stderr,"\n\n********************************\n\n");
#endif
#ifndef HDUEPP
  fprintf(stderr,"\n\n********************************\n\n");
  ...
  data_shm->reset_dsp_and_synth = 1; // so that first time we try to do an acquisition, it will happen. // TODO is this needed? I don't think it is.
  init_sched();
...
// hmm, this should really be in pulse-duesdr.c?
double ppo_time(){
  double how_long=0.;
  int i;
